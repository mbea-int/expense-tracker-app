...
def no_git_env(_env: Optional[MutableMapping[str, str]] = None,) -> Dict[str, str]:
...
def get_root() -> str:
    ...
    try:
        root = os.path.abspath(cmd_output("git", "rev-parse", "--show-cdup")[1].strip())
...
def get_changed_files(old: str, new: str) -> List[str]:
    return zsplit(
        cmd_output(
            "git", "diff", "--name-only", "--no-ext-diff", "-z", f"{old}...{new}"
        )[1]
    )
