...
def get_env_patch(venv: str, language_version: str) -> PatchesT:
    ...
    if language_version == "system":
        patches += (
            ("PATH", (os.path.join(venv, "gems", "bin"), os.pathsep, Var("PATH"))),
        )
...
def in_env(prefix: Prefix, language_version: str) -> Generator[None, None, None]:
    envdir = prefix.path(helpers.environment_dir(ENVIRONMENT_DIR, language_version))
...
def _install_rbenv(prefix: Prefix, version: str) -> None:  # pragma: win32 no cover
...
def _install_ruby(prefix: Prefix, version: str) -> None:  # pragma: win32 no cover
    try:
...
def install_environment(
    ...
    with clean_path_on_failure(prefix.path(directory)):
        ...
        with in_env(prefix, version):
            helpers.run_setup_cmd(prefix, ("gem", "build", *prefix.star(".gemspec")))
