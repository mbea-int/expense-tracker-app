...
def filter_by_include_exclude(
    names: Collection[str], include: str, exclude: str
) -> List[str]:
...
class Classifier:
    ...
    def filenames_for_hook(self, hook: Hook) -> Tuple[str, ...]:
        ...
        names = self.by_types(names, hook.types, hook.types_or, hook.exclude_types)
        return tuple(names)

    @classmethod
    def from_config(
        cls, filenames: Collection[str], include: str, exclude: str
    ) -> "Classifier":
...
def _has_unstaged_config(config_file: str) -> bool:
    retcode, _, _ = cmd_output_b(
        "git", "diff", "--no-ext-diff", "--exit-code", config_file, retcode=None
    )
