...
class Store:
    ...
    def _new_repo(
        ...
        def _get_result() -> Optional[str]:
            ...
            with self.connect() as db:
                result = db.execute(
                    "SELECT path FROM repos WHERE repo = ? AND ref = ?", (repo, ref)
                ).fetchone()
    ...
    def make_local(self, deps: Sequence[str]) -> str:
        ...
        return self._new_repo("local", C.LOCAL_REPO_VERSION, deps, make_local_strategy)
    ...
    def delete_repo(self, db_repo_name: str, ref: str, path: str) -> None:
        with self.connect() as db:
            db.execute(
                "DELETE FROM repos WHERE repo = ? and ref = ?", (db_repo_name, ref)
            )
        rmtree(path)
