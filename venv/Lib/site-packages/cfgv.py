...
OptionalNoDefault = collections.namedtuple("OptionalNoDefault", ("key", "check_fn"))
...
ConditionalOptional = collections.namedtuple(
    "ConditionalOptional",
    ("key", "check_fn", "default", "condition_key", "condition_value", "ensure_absent"),
)
...
WarnAdditionalKeys = collections.namedtuple("WarnAdditionalKeys", ("keys", "callback"))
...
class Array(collections.namedtuple("Array", ("of", "allow_empty"))):
    ...
    def check(self, v):
        ...
        if not self.allow_empty and not v:
            raise ValidationError(f"Expected at least 1 '{self.of.object_name}'")
        for val in v:
...
def check_type(tp, typename=None):
    def check_type_fn(v):
        if not isinstance(v, tp):
            ...
            raise ValidationError(f"Expected {typename_s} got {type(v).__name__}")
...
def check_one_of(possible):
    def check_one_of_fn(v):
        if v not in possible:
            ...
            raise ValidationError(f"Expected one of {possible_s} but got: {v!r}")
...
def check_array(inner_check):
    def check_array_fn(v):
        if not isinstance(v, (list, tuple)):
            raise ValidationError(f"Expected array but got {type(v).__name__!r}")
...
def load_from_filename(filename, schema, load_strategy, exc_tp=ValidationError):
