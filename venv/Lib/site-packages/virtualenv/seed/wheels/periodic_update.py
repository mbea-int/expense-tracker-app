...
def periodic_update(
    distribution, for_py_version, wheel, search_dirs, app_data, do_periodic_update, env
):
    ...
    u_log_older_than_hour = (
        now - u_log.completed > timedelta(hours=1)
        if u_log.completed is not None
        else False
    )
...
class NewVersion(object):
    ...
    def __eq__(self, other):
        return type(self) == type(other) and all(
            ...
            for k in ["filename", "release_date", "found_date"]
        )
...
def _run_do_update(
    ...
    while last is None or not last.use(now):
        ...
        last = NewVersion(
            filename=dest.path.name, release_date=release_date, found_date=download_time
        )
...
def _run_manual_upgrade(app_data, distribution, for_py_version, env):
    ...
    args = [distribution, for_py_version, datetime.now() - start]
    if versions:
