...
class CreatorSelector(ComponentBuilder):
    ...
    def for_interpreter(cls, interpreter):
        ...
        if not key_to_meta:
            if errors:
                rows = [
                    "{} for creators {}".format(k, ", ".join(i.__name__ for i in v))
                    for k, v in errors.items()
                ]
                raise RuntimeError("\n".join(rows))
            else:
                raise RuntimeError(
                    "No virtualenv implementation for {}".format(interpreter)
                )
        return CreatorInfo(
    ...
    def add_selector_arg_parse(self, name, choices):
        ...
        self.parser.add_argument(
            ...
            help="create environment via{}".format(
                ""
                if self.builtin_key is None
                else " (builtin = {})".format(self.builtin_key)
            ),
        )
