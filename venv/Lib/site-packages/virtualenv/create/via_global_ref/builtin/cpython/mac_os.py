...
class CPythonmacOsFramework(CPython):
    ...
    def can_describe(cls, interpreter):
        return is_mac_os_framework(interpreter) and super(
            CPythonmacOsFramework, cls
    ...
    def _executables(cls, interpreter):
        for _, targets, must, when in super(CPythonmacOsFramework, cls)._executables(
            interpreter
        ):
...
def fix_mach_o(exe, current, new, max_size):
    ...
    try:
        logging.debug(
            u"change Mach-O for %s from %s to %s",
            ensure_text(exe),
            current,
            ensure_text(new),
        )
...
def _builtin_change_mach_o(maxint):
    ...
    class FileView(object):
        ...
        def __repr__(self):
            return "<fileview [{:d}, {:d}] {!r}>".format(
                self._start, self._end, self._file_obj
            )
        ...
        def _checkwindow(self, seek_to, op):
            if not (self._start <= seek_to <= self._end):
                msg = "{} to offset {:d} is outside window [{:d}, {:d}]".format(
                    op, seek_to, self._start, self._end
                )
                raise IOError(msg)
    ...
    def mach_o_change(at_path, what, value):
        ...
        def do_macho(file, bits, endian):
            ...
            cpu_type, cpu_sub_type, file_type, n_commands, size_of_commands, flags = read_data(
                file, endian, 6
            )
