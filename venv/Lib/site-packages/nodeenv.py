...
def parse_args(check=True):
    ...
    parser = optparse.OptionParser(
        version=nodeenv_version, usage="%prog [OPTIONS] ENV_DIR"
    )
...
def get_node_bin_url(version):
    ...
    sysinfo = {"system": platform.system().lower(), "arch": archmap[platform.machine()]}
...
def build_node_from_src(env_dir, src_dir, node_src_dir, opt):
    ...
    make_param_values = map(
        lambda x: getattr(opt, x.replace("-", "_")), make_param_names
    )
    make_opts = [
...
def install_node_wrapped(env_dir, src_dir, opt):
    ...
    logger.info(
        " * Install %s node (%s) " % (src_type, opt.node), extra=dict(continued=True)
    )
...
def install_npm(env_dir, _src_dir, opt):
    ...
    and install it in virtual environment.
    """
    logger.info(" * Install npm.js (%s) ... " % opt.npm, extra=dict(continued=True))
    env = dict(
        os.environ, clean="no" if opt.no_npm_clean else "yes", npm_install=opt.npm
    )
    proc = subprocess.Popen(
        (
            "bash",
            ...
            ". {0} && npm install -g npm@{1}".format(
                pipes.quote(join(env_dir, "bin", "activate")), opt.npm
            ),
        ),
        env=env,
...
def install_packages(env_dir, opt):
    ...
    packages = [package.strip() for package in open(opt.requirements).readlines()]
    ...
    for package in packages:
        if not package:
            continue
        callit(cmd=[cmd % {"pack": package}], show_stdout=opt.verbose, in_shell=True)
